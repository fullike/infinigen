...
def home_room_constraints(fast=False):
    ...
    if fast:
        node_constraint = (
            ...
            * (rooms[Semantics.DiningRoom].count() >= 1)
    ...
    room_term = (
        ...
        + (
            ...
            + rooms[Semantics.DiningRoom].sum(
                lambda r: (r.area() / 60).log().hinge(0, 0.4).pow(2)
            )
...
def home_furniture_constraints():
    ...
    constraints["diningroom"] = diningrooms.all(
        lambda r: (
            (diningtables.related_to(r).count().in_range(0, 8))
            * storage.related_to(r).all(
                lambda t: (
    ...
    score_terms["diningroom"] = diningrooms.mean(
        lambda r: (
            diningtables.related_to(r).count().maximize(weight=16)
            + diningtables.related_to(r).sum(lambda t: t.distance(r)).maximize(weight=1)
            + diningtables.related_to(r).mean(lambda t: t.distance(diningtables.related_to(r))).maximize(weight=1)
            )
        )
    # endregion
